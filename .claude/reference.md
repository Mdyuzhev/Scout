...
## Сравнение моделей для scout_brief: Haiku vs Sonnet vs Opus

Эксперимент проведён на SC-017.2 (рынок недвижимости): один и тот же контекст
(15 чанков, top_k=15), три разных модели, одинаковый запрос.

| Метрика | Haiku | Sonnet | Opus |
|---------|-------|--------|------|
| Токены | 25 362 | 24 574 | 24 552 |
| Длина | ~4 000 симв. | ~4 490 симв. | ~4 249 симв. |
| Разделов | 5 | 3 | 5 |

**Уникальные находки Haiku** (только у него):
- Ставки по ипотеке с динамикой (19–28% → 16,5% → прогноз 12–14%)
- Требования к заёмщикам: стаж 1 год, первоначальный взнос 25%
- Разбивка инвестиций по сегментам (офисы +310%, склады +70%, ритейл −69%)
- Региональная детализация: Удмуртия +18,4%, Ленобласть +15,1%
- Вторичный рынок Москвы: конкретная цена 314,1 тыс. руб./м²

**Уникальные находки Sonnet** (только у него):
- Логика восстановления H2 2025 — три конкретных фактора с механикой
- Доля рыночной ипотеки в Москве: с <10% → 21% (октябрь)
- Альтернативные сделки как доминирующий тип

**Уникальные находки Opus** (только у него):
- Прогноз инвестиций на 2025 и 2026: ~1 трлн руб. (−24%), затем 800–900 млрд
- Региональные различия спроса: МСК ≈ 2024, СПб −15%, миллионники −12%
- Конкретные города с max сокращением предложения (Киров, Уфа: −40%)
- Переток средств с депозитов как фактор восстановления
- Ижевск, Чебоксары в лидерах роста вторичного рынка

**Общее у всех трёх**: инвестиции 1,2 трлн / +41%, площадки +83%, 25 млн м²,
ноябрьские цифры (59 тыс. семейная + 40 тыс. рыночная), Севастополь +68%.

**Характер синтеза:**
- Haiku — самый «фактурный»: много конкретных цифр, структура плоская, связей нет
- Sonnet — лучший нарратив: объясняет «почему», строит причинно-следственные цепочки
- Opus — лучший баланс: структура как у Sonnet + детализация как у Haiku

**Рекомендации по выбору модели:**
- Для отчётов и баз данных с конкретными цифрами → **Haiku** (дешевле, фактурнее)
- Для стратегических обзоров и питчей → **Sonnet** (нарратив, причинность)
- Для максимально полного аналитического брифа → **Opus** (баланс цифр и смысла)
- При ограниченном бюджете: Haiku с top_k=20 компенсирует часть глубины Opus

Важное наблюдение: все три модели получили **одинаковый контекст** — разница
объясняется только тем как каждая модель обрабатывает один и тот же текст.
Haiku "верен тексту" и вытаскивает цифры буквально. Sonnet и Opus активнее
задействуют внутренние знания для построения нарратива, но часть буквальных
цифр при этом могут не включить явно.
