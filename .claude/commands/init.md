...
**Шаг 4:** `get_services_status` — получить статус scout-контейнеров
→ из ответа извлечь scout-mcp и scout-postgres
→ написать: **✅ Шаг 4 готов — сервисы проверены**
...
```
════════════════════════════
🏠 Сессия #N | 🟢 LAN / 🟡 Tailscale / 🔴 MCP недоступен
📦 scout-mcp:      ✅ running (Xms) / ❌ stopped
📦 scout-postgres:  ✅ healthy / ❌ stopped
📋 Предыдущая сессия: [итог из get_context или "нет данных"]
📝 Backlog: [задачи из контекста/CLAUDE.md]
════════════════════════════
```
...
`⚠️ Продолжить с paramiko-fallback? (да/нет)`
